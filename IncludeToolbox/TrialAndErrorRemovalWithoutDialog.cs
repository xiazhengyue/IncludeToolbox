...
namespace IncludeToolbox
{
    ...
    /// </summary>
    internal sealed class TrialAndErrorRemovalWithoutDialog
    {
        ...
        public async Task<bool> PerformTrialAndErrorIncludeRemoval(EnvDTE.Document document, TrialAndErrorRemovalOptionsPage settings)
        {
            ...
            if (WorkInProgress)
            {
                Output.Instance.WriteLine("Trial and error include removal already in progress!");
                return false;
            }
            WorkInProgress = true;
                ...
                foreach (Formatter.IncludeLineInfo line in includeLines)
                {
                    ...
                    if (!noTimeout || !lastBuildSuccessful)
                    {
                        ...
                        ThreadHelper.JoinableTaskFactory.Run(async () =>
                        {
                            ...
                            document.Save();
                            if (!noTimeout)
                                Output.Instance.WriteLine("Compilation of {0} timeouted!", document.Name);
                        });
